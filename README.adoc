...
At a chosen interval it sends the following NMEA0183 sentences
...
=== If there is an active waypoint, it also sends

* RMB
* BOD

At a chosen interval it sends

* WPL for each waypoint in the route ensuring the waypoint name is unique within the route
* RTE as many as required to define the route by linking the waypointst

If the first waypoint in the route has not been reached, the script prepends a leg from the point of activation to the first waypoint
